...
_EXPANSION_TOKEN = int(BasisToken.EXPANSION)


def FROZEN_COST_FN(grid, invariants):
    return (
        len(
            _authority_residual(
                tuple(grid),
...
def _stable_guidance_order(
    ...
    *,
    ...
    ordered = sorted(
        before,
        key=lambda move: -_move_proposal_agreement(
            current,
            move,
            proposal,
            apply_fn=apply_fn,
    ...
    def __call__(
        ...
        with torch.inference_mode():
            _, proposed_tensor, _ = self.model.propose(
                ...
                _grid_tensor(ri.writable_mask),
                _bits_tensor(ri.declared_features),
                _bits_tensor(ri.active_residual),
                carry=None,
            )
...
def _proposal_order(
    *,
    ...
    ordered_moves = _stable_guidance_order(
        ri.grid81,
        moves,
        proposal,

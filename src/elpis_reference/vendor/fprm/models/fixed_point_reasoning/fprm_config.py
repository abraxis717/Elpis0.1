...
class FPRMConfig(ReasoningModelConfig):
    ...
    # Loop attention: at each loop step the current state attends over earlier
    # loop sources (the loop index is the attention axis). `mode` picks what the
    # history holds; `window` caps how many earlier loops are kept.
    loop_attnres: bool = False
    loop_attnres_mode: Literal["state", "delta"] = "state"
    loop_attnres_window: int = 16
    # "lite" = single learned pseudo-query (Kimi AttnRes); "mha" = multi-head with
    # K/V/O projections + learned per-head query (stronger routing).
    loop_attnres_impl: Literal["lite", "mha"] = "lite"
    # If True, AttnRes is a gated CORRECTION on top of the clean residual
    # (y = (1-g)*(x+o_last) + g*attn), gate g~0 at init so it starts exactly as
    # the residual baseline and learns to add history routing. If False, AttnRes
    # fully replaces the residual (original behaviour).
    loop_attnres_residual_mix: bool = False

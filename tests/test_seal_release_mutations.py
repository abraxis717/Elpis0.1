...
def test_published_belt_is_independent_of_manifest_existence(tmp_path: Path) -> None:
    root = copy_repo(tmp_path)
    (root / "VERSION").write_text("2.1.12\n", encoding="utf-8")
    manifest = root / "manifests/Elpis2.1.12.RELEASE_MANIFEST.json"
    manifest.unlink(missing_ok=True)

    proc = run(root, str(SEALER), "--version", "2.1.12")

    assert proc.returncode == 2
    assert "Elpis2.1.12 is published; its manifest is immutable" in output(proc)

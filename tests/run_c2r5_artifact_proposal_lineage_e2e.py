...
def main():
    ctx=RequestContext(request_id="c2r5-production-lineage-e2e",prompt="write deterministic typed python solution and validate without imports",domain="python",entrypoint="solution",parameters=("x",))
    ...
    trace=build_p0_projection_trace(projection_digest=result.projection.digest,grid81=result.projection.grid81,semantic_rows=result.projection.semantic_rows)
    target=next(c for c in range(63,72) if result.projection.grid81[c]!=0)
    unrelated=next(c for c in range(0,63) if result.projection.grid81[c]!=0)
    state=ClampState.empty(ctx.request_id)
    initial=apply_clamp_transaction(state=state,transaction=ClampTransaction(transaction_id="prevalidation-support",episode_id=state.episode_id,expected_state_digest=state.digest(),proposals=(ClampProposal(proposal_id="validation",operation=ClampOperation.ASSERT,slot_id="p0.validation.support",evidence_digest=trace.trace_digest,cell_index=target,value=result.projection.grid81[target]),ClampProposal(proposal_id="unrelated",operation=ClampOperation.ASSERT,slot_id="p0.unrelated.support",evidence_digest=trace.trace_digest,cell_index=unrelated,value=result.projection.grid81[unrelated]))))
    ...
    diagnostic=task_diagnostic_from_p0_validator_failure(task_scope_id=ctx.request_id,frame_index=0,artifact_digest=result.artifact.digest,evidence=result.evidence[0],projection_trace=trace,lineage=lineage)
    ...
    report={
        ...
        "release":{
            ...
            "unrelated_support_preserved":bool(released.state.active_mask[unrelated]),
        },
        "claims":{
            ...
            "validator_ingress_requires_verified_lineage":True,

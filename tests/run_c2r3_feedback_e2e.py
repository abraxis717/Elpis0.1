...
SOLVED_TEXT=("534678912672195348198342567859761423426853791713924856" "961537284287419635345286179")
...
def main():
    ...
    hypothesis_owner="c2r4-independent-hypothesis"
    assertions=[]
    for cell,value in enumerate(SOLVED):
    ...
    semantic=h("c2r4-independent-hypothesis-locus")
    ...
    diagnostic=TaskDiagnosticV1(
        ...
        subject_digest=samsung_proposal_digest(SOLVED), producer_id="c2r4.fixture-task-validator.v1",
        locus_namespace=SEMANTIC_OBJECT, locus_identity=semantic,
        reason_codes=("TASK_REQUIREMENT_UNSATISFIED",), details_digest=h("fixture-task-evidence"),
    )
    traversal=execute_samsung_feedback_step(
        run_id="c2r4-reference-e2e", refinement_step_index=0, prior_proposal=SOLVED,
    ...
    if traversal.released_cells != (0,): raise RuntimeError("release was not exactly the independent hypothesis")
    ...
    surviving=all(traversal.learned_solution[cell]==SOLVED[cell] for cell in range(1,81))
    ...
    report={
      ...
      "released_value_source":"independent_fixture_not_model_output",

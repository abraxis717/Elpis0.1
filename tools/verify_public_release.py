#!/usr/bin/env python3
"""Fail-closed public verifier for Elpis2.1.1."""
...
RELEASE_MANIFEST_REL = Path(
    "manifests/Elpis2.1.1.RELEASE_MANIFEST.json"
)

DISTRIBUTION_MANIFEST_REL = Path(
    "manifests/Elpis2.1.1.DISTRIBUTION_MANIFEST.json"
)
...
def load_manifest():
    ...
    expected = {
        ...
        "release_name": "Elpis2.1.1",
        "release_tag": "Elpis2.1.1",
        "version": "2.1.1",
        "package_name": "elpis",
    ...
    if MANIFEST_REL == DISTRIBUTION_MANIFEST_REL:
        ...
        expected["distribution_version"] = "2.1.1"
...
def check_package():
    ...
    if project.get("version") != "2.1.1":
        errors.append("package version is not 2.1.1")

    if (REPO / "VERSION").read_text().strip() != "2.1.1":
...
def main() -> int:
    ...
    if passed:
        print(
            "PASS: Elpis2.1.1 public release verified"
        )
        return 0

    print(
        "FAIL: Elpis2.1.1 public release verification failed"
    )
    return 1

...
int elpis_refinement_backend_registry_identity(
    ...
    static const char domain[] = "elpis.semantic.refinement_backend_registry.v2";
    ...
    { uint32_t encoded_u32 = __builtin_bswap32(r->backend_count); memcpy(buf + off, &encoded_u32, 4); off += 4; }
    for (uint32_t i = 0; i < r->backend_count; i++) {
